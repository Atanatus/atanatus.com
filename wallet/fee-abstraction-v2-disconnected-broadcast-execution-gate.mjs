...
export const ATAN_FEE_V2_W28_VERSION =
  '1.0.0';
...
function requireTransactionHex(value){
  ...
  if(
    ...
    fail(
      'W28_SIGNED_TRANSACTION_HEX_INVALID'
    );
  }
...
function canonicalizeLocal(value){
  ...
  if(typeof value === 'number'){
    if(!Number.isFinite(value)){
      fail(
        'W28_NONFINITE_NUMBER_FORBIDDEN'
      );
    }
...
export async function createDisconnectedBroadcastExecutionGateV2({
  ...
  if(typeof verify_owner_signature !== 'function'){
    fail(
      'W28_OWNER_SIGNATURE_VERIFIER_REQUIRED'
    );
  }
  ...
  if(
    ...
    fail(
      'W28_ACTIVE_DURABLE_BROADCAST_CLAIM_REQUIRED'
    );
  }
  ...
  if(
    ...
    fail(
      'W28_DURABLE_CLAIM_SAFETY_STATE_INVALID'
    );
  }
  ...
  if(
    ...
    fail(
      'W28_DURABLE_CLAIM_AUTHORIZATION_BINDING_MISMATCH'
    );
  }
  ...
  if(
    ...
    fail(
      'W28_SIGNED_TRANSACTION_BYTE_LENGTH_MISMATCH'
    );
  }
  ...
  if(actualTxHash !== txHash){
    fail(
      'W28_SIGNED_TRANSACTION_HASH_MISMATCH'
    );
  }
  ...
  if(
    currentHeight >
      Number(
        durable_claim_receipt.valid_until_height
      )
  ){
    fail(
      'W28_DURABLE_BROADCAST_CLAIM_EXPIRED'
    );
  }
...
export function verifyDisconnectedBroadcastExecutionGateV2(
  ...
  if(
    ...
    fail(
      'W28_EXECUTION_GATE_STATE_INVALID'
    );
  }
  ...
  if(
    ...
    fail(
      'W28_EXECUTION_GATE_ID_MISMATCH'
    );
  }
...
export async function attemptDisconnectedBroadcastExecutionV2(
  ...
  fail(
    'W28_BROADCAST_TRANSPORT_DISCONNECTED'
  );
}
